#today we will be getting a null distribution/related to the null hypothisis 
#the ditribution will be     is the null is true
#in battary test ho= mug=mub
#test stat x bar of g - x bar of b 
...
# Last Updated 03/16/2015 @ 07:30pm

Batt <- read.csv(file=url("http://www1.appstate.edu/~thomleyje/R-datafiles/BatteryLife.csv"))

# Print out observed data and descriptives comparing the groups.

data.frame(Generic=Batt$Hours[7:12],
           Brand=Batt$Hours[1:6])
boxplot(Hours~Type, data=Batt, horizontal=T)
tapply(Batt$Hours, Batt$Type, mean)
tapply(Batt$Hours, Batt$Type, sd)

# How many permutations would we have in an exact test?

print(paste("The number of exact permutations is ", choose(12,6)),quote=F)
...
print(paste("The observed mean difference (generic - brand) is ", round(obs.diff,4), ".", sep=""),quote=F)
...
# The vector "results" stores the sampled null distribution; it
...
for(i in 1:N)
{
  index <- sample(12, size=6)
  results[i] <- mean(life[index]) - mean(life[-index])
}
...
pvalue.upper<-format((sum(results>=obs.diff)+1)/(N+1),scientific=F)
print(paste("The upper-tailed p-value for Ha: generic mean > brand mean is ", pvalue.upper, ".", sep=""),quote=F)
